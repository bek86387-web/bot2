...
def handle_analysis(message):
    ...
    try:
        ...
        ax.set_title(f"{ticker} Professional Gann Analysis", color='white', fontsize=18, pad=30)
        buf = io.BytesIO()
        ...
        bot.send_photo(message.chat.id, buf, caption=f"✅ **{ticker} Professional Tahlili**\n💰 **Narx:** `{current_price}$`", parse_mode='Markdown')
